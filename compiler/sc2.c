 ...
 *
 *  Version: $Id: sc2.c,v 1.1.1.1 2003/04/26 19:59:38 luxornox Exp $
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
static void doinclude(void)
{
  ...
  check_empty(lptr+1);
  /* Include files between "..." are read from the current directory, or from
   * a list of "include directories". Include files between <...> are only read
   * from the list of include directories
   */
  result=0;
  if (c=='\"')
    result=plungefile(name);
  for (i=0; !result && (ptr=get_path(i))!=NULL && name[0]!=DIRSEP_CHAR; i++) {
    char path[_MAX_PATH];
    strcpy(path,ptr);
    strcat(path,name);
    result=plungefile(path);
  } /* while */
  if(!result)
    ...
    if (skiplevel==0) {
      if (lex(&val,&str)==tSYMBOL) {
        ...
        while (*lptr<=' ' && *lptr!='\0')
          lptr++;
        if (*lptr!='\0')
          preproc_expr(&val,&tag);  /* get constant expression */
        else
          val=0;
  ...
  *lexsym=_lexstr;
  ...
  if ((i=number(&_lexval,lptr))!=0) {   /* number */
    ...
    *lexvalue=_lexval=litidx;
    ...
    lptr=sc_packstr ? packedstring(lptr) : unpackedstring(lptr);
    if (*lptr=='\"')
    ...
    *lexvalue=_lexval=litidx;
    ...
    lptr=sc_packstr ? unpackedstring(lptr) : packedstring(lptr);
    if (*lptr=='\"')
      lptr+=1;          /* skip final quote */
    else
      error(37);        /* invalid (non-terminated) string */
...
static cell litchar(char **lptr)
{
  ...
  if (*cptr!=sc_ctrlchar) {     /* the caret identifies a control character */
    c=*cptr;
    cptr+=1;
  } else {
    cptr+=1;
...
static void free_symbol(symbol *sym)
{
  ...
  if (sym->ident==iFUNCTN) {
    ...
    for (arg=sym->dim.arglist; arg->ident!=0; arg++) {
      ...
      else if (arg->ident==iVARIABLE && (arg->hasdefault & uSIZEOF)!=0)
        free(arg->defvalue.symname);
