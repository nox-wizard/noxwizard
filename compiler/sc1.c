 ...
 *
 *  Version: $Id: sc1.c,v 1.1.1.1 2003/04/26 19:59:34 luxornox Exp $
 */
#include <assert.h>
#include <ctype.h>
#include <limits.h>
...
// Akron Fix
// this is for Borland C for DOS.
// I need to use only the windows version
#if defined WIN32
	#include <direct.h>
  ...
  do {
    ...
    if (!fstatic) {             /* static variables already initialized */
      if (ident==iVARIABLE) {
        ...
        lval.constval=0;
        store(&lval);
        endexpr();              /* full expression ends after the store */
...
static void decl_enum(int vclass)
{
  ...
  /* get optional enum name (also serves as a tag) */
  if (lex(&val,&str)==tSYMBOL) {        /* read in (new) token */
                   ...
                   int *tag,int chkfuncresult)
{
  ...
  if (mark_endexpr)
    endexpr();                  /* optionally, mark the end of the expression */
  errorset(sEXPRRELEASE);
  if (localstaging) {
    stgout(index);
...
static void test(int label,int parens,int invert)
{
  ...
  endexpr();                    /* end expression (give optimizer a chance) */
...
#pragma warning(pop)
  ...
  if (nestlevel>save_nestlevel) {
    ...
    declared=save_decl;
    delete_symbols(&loctab,nestlevel,TRUE,TRUE);
