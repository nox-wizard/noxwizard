 ...
 *
 *  Version: $Id: sc3.c,v 1.1.1.1 2003/04/26 19:59:43 luxornox Exp $
 */
#include <assert.h>
#include <stdio.h>
...
static void (*unopers[])(void) = { lneg, neg, user_inc, user_dec };
  ...
  *resulttag=sym->tag;          /* save tag of the called function */
  ...
  if (savepri)
    pop2();                     /* restore PRI that was saved into ALT */
  if (lval!=NULL) {
    if (lval->ident==iARRAYCELL || lval->ident==iARRAYCHAR)
...
static cell arraysize(symbol *sym)
{
  ...
  if (sym->dim.array.level > 0) {
    cell sublength=arraysize(finddepend(sym));
    if (sublength>0)
...
static int hier1(value *lval1)
{
  int lvalue,index,tok,symtok,close;
  cell val,cidx;
  value lval2;
  char *st;
  symbol *sym;
...
static int primary(value *lval)
{
  ...
  if (matchtoken('(')){         /* sub-expression - (expression,...) */
    ...
    while (matchtoken(','));
    needtoken(')');
    lexclr();                   /* clear lex() push-back, it should have been
...
static void callfunction(symbol *sym)
{
  ...
  if (!matchtoken(')')) {
    do {
      ...
      stgmark(sEXPRSTART+(char)argpos);/* mark beginning of new expression in stage */
  ...
  for (argidx=0; arg[argidx].ident!=0 && arg[argidx].ident!=iVARARGS; argidx++) {
    ...
    /* in this first stage, we also skip the arguments with uSIZEOF; these are
     * handled last
     */
    ...
    stgmark(sEXPRSTART+(char)argidx); /* mark beginning of new expression in stage */
    if (arg[argidx].hasdefault) {
      if (arg[argidx].ident==iREFARRAY) {
        ...
        append_constval(&arrayszlst,arg[argidx].name,arg[argidx].defvalue.array.size);
  ...
  for (argidx=0; arg[argidx].ident!=0 && arg[argidx].ident!=iVARARGS; argidx++) {
    ...
    assert((arg[argidx].hasdefault & uSIZEOF)!=0);  /* if unseen, must be "sizeof" */
     ...
     */
    asz=find_constval(&arrayszlst,arg[argidx].defvalue.symname);
    if (asz!=NULL) {
      array_sz=asz->value;
      if (array_sz==0)
