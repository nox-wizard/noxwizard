...
//Endymion Save new Guild System
	Guildz.safeoldsave();
	fileName = SrvParms->savePath + SrvParms->guildWorldfile + SrvParms->worldfileExtension;
	gWsc=fopen( fileName.c_str(), "w");
	if (!gWsc)
	{
		ErrOut("Error, couldn't open %s for writing. Check file permissions.\n", fileName.c_str() );
		tempfx::tempeffectson();
	...
	fprintf(gWsc, "// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
	fprintf(gWsc, "// || NoX-Wizard guild save (nxwguild.wsc)                                ||\n");
	fprintf(gWsc, "// || Automatically generated on worldsaves                               ||\n");
	fprintf(gWsc, "// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
...
// Begin of Guild related things (DasRaetsel)
      ...
      #define SAVECHAREVENT(A,B) if ((pc->amxevents[B])&&(pc->amxevents[B]->shouldBeSaved())) fprintf(cWsc, "%s %s\n", A, pc->amxevents[B]->getFuncName());
			...
			SAVECHAREVENT("@ONDEATH", EVENT_CHR_ONDEATH);
...
/*
 Sparhawk 2001-09-20 added savecharevents onblock and onstart onheartbeat
*/
...
// Sparhawk added on single click event
