...
cell AmxProgram::CallFn (int idx, int param1, int param2, int param3, int param4, int param5, int param6)
{
	...
	try {
	  ...
	  err = amx_Exec(m_AMX, &ret, idx, 6, par1, par2, par3, par4, par5, par6);
		...
		if ( err != AMX_ERR_NONE )
			ConOut("Got error %d during amx execution of function", err);
	  return ret;
		...
		ConOut("Exception Handled during AMX Script Execution : skipping script %s\n", amx_FindFunctionbyIndex(amx, idx));
		return -1;
	}
}
...
/*!
\brief calls an amx function with 8 integer parameters
\author Endymion
\return cell
\param idx function index (as returned by funcidx())
\param param1 parameter 1
