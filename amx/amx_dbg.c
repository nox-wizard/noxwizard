...
#define NAMEMAX         64      /* length of variable/function names */
#define MAX_DIMS        2       /* number of array dimensions */
#define TABSIZE         8
#ifdef _WINDOWS
#define SCREENCOLUMNS   254
#else
#define SCREENCOLUMNS   80
#endif
#define MAXWATCHES    16
#define SCREENLINES   40      /* number of lines that fit on a screen */
#define DEF_LIST      20      /* default length of code list */
...
static void term_csrget(int *x,int *y)
{
  ...
  val=dgetchar();
  assert(val=='[');
  for (i=0; i<8 && (val=dgetchar())!=';'; i++)
    str[i]=(char)val;
  str[i]='\0';
  *y=atoi(str);
  for (i=0; i<8 && (val=dgetchar())!='R'; i++)
    str[i]=(char)val;
  str[i]='\0';
  *x=atoi(str);
  val=dgetchar();
...
static void source_list(int startline, int numlines)
{
  ...
  if (cursource[result]!=NULL) {
    ...
    while (cursource[startline]!=NULL && startline<lastline) {
      ...
      if (terminal==TERM_ANSI)
        term_clreol();
      if (strlen(cursource[startline])>SCREENCOLUMNS-TABSIZE) {
        c1=cursource[startline][SCREENCOLUMNS-TABSIZE-1];
        c2=cursource[startline][SCREENCOLUMNS-TABSIZE];
        cursource[startline][SCREENCOLUMNS-TABSIZE-1] = '\n';
        cursource[startline][SCREENCOLUMNS-TABSIZE] = '\0';
      } /* if */
      if (startline==curline)
        dprintf("=>[%4d]\t%s",startline+1,cursource[startline]);
      else
        dprintf("   %4d \t%s",startline+1,cursource[startline]);
      if (c1!='\0') {
        cursource[startline][SCREENCOLUMNS-TABSIZE-1] = c1;
        cursource[startline][SCREENCOLUMNS-TABSIZE] = c2;
      } /* if */
      startline++;
    } /* while */
  } /* if */
...
static void watch_list(AMX *amx,int calllevel)
{
  ...
  for (i=num=0; i<MAXWATCHES; i++) {
    if (strlen(watches[i])>0) {
      ...
      dprintf("%d   %s\t",i+1,strpatch(watches[i]));
  ...
  if (terminal==TERM_ANSI) {
    ...
    if (num>0)
		dprintf("___________________________________________________________________________________\n");
    } /* if */
}
...
static int break_check(AMX *amx,int line,int file)
{
  ...
  for (index=0; index<MAXBREAKS; index++) {
    if (breakpoints[index].type==BP_CODE && breakpoints[index].addr==(unsigned)(line)
...
static void listcommands(char *command)
{
  ...
  } else if (stricmp(command,"cwatch")==0 || stricmp(command,"cw")==0) {
             ...
             || stricmp(command,"s")==0 || stricmp(command,"step")==0)
  {
    ...
    dprintf("\tTERM ANSI\tuse VT100/ANSI terminal display\n"
...
static char lastcommand[10] = "";
  ...
  for ( ;; ) {
	...
	if (curfileno>=0) dprintf("%s(%d)# ", filenames[curfileno], curline+1);
	else dprintf("- ");
      ...
      if (*params=='\0') {
        ...
        for (sym=vartab.next; sym!=NULL; sym=sym->next) {
          if (sym->calllevel==-1 || sym->calllevel==calllevel) {
            dprintf("%s\t%s\t",sym->vclass>0 ? "loc" : "glb",strpatch(sym->name));
            if (sym->type==3 || sym->type==4) {
...
#if defined __INDEPENDANT_DEBUGZD
