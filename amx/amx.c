 ...
 *
 *  Version: $Id: amx.c,v 1.1.1.1 2003/04/26 19:58:52 luxornox Exp $
 */

//XAN
int g_nTraceMode=0; //by xan -> trace mode

#include "console.h"	// added by Anthalir for ConOut
...
#define NUMENTRIES(hdr,field,nextfield) \
                        (int)(((hdr).nextfield - (hdr).field)/sizeof(AMX_FUNCSTUB))
...
int AMXAPI amx_Callback(AMX *amx, cell index, cell *result, cell *params)
{
  ...
  *result = f(amx,params);
  if (g_nTraceMode!=0) {
      pars = params[0]/sizeof(cell);
      STraceOut("native %s:%d(", func->name,pars);
      maxpar = (((pars) < (8)) ? (pars) : (8));
      if (maxpar>0) {
          for (i = 1; i<=maxpar; i++)
          {
              if (i!=1) STraceOut(",");
              STraceOut("%d", params[i]);
          }
          if (maxpar<pars) STraceOut(",...");
      }
      STraceOut(") returned value %d\n", *result);
  }
