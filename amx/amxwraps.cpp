...
/*
\brief Add a new GM page call
\author Frodo
\since 0.82
\param 1 serial of the char
\param 2 reason 
*/
...
NATIVE(_makeGMpage)
{
	...
	P_CHAR pc_currchar = pointers::findCharBySerial( params[1] );
	...
	case T_CHAR : {
		...
		for(i=1;i<MAXPAGES;i++)
		{
			if(gmpages[i].handled==1)
			{
				...
				gmpages[i].serial.serial32 = params[1];
				time_t current_time = time(0);
				struct tm *local = localtime(&current_time);
				sprintf(gmpages[i].timeofcall, "%02d:%02d:%02d", local->tm_hour, local->tm_min, local->tm_sec);
				sprintf(temp,"%s [ %08x ] called at %s, %s",gmpages[i].name,params[1],gmpages[i].timeofcall,gmpages[i].reason);
		...
		if (x2==0)
			...
			if(reason == "OTHER")
				...
				pc_currchar->getCurrentNameC(), params[1], reason.c_str());
				bool found=false;
	...
	default : {
				ErrOut("_makeGMpage called with invalid property %s!\n", params[2] );
				break;
			  }
...
/*
\brief Returns the amount of GM pages
\author Frodo
\since 0.82
*/

NATIVE (_GMpagesAmount)
{
	int i;
	
	for(i=1;gmpages[i].handled==0;i++)
...
/*
\brief Return the serial of the char who made the page
\author Frodo
\since 0.82
\param 1 number of the page
*/


/*
\brief Return the reason of a GMpage made by a char
\author Frodo
\since 0.82
\param 1 serial of the char
\param 2 reason 
...
// GM Pages functions - Frodo
 { "makeGMpage",     _makeGMpage },
 { "GMpagesAmount",  _GMpagesAmount },
// Terminator :
 { NULL, NULL }
};
