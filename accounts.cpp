...
namespace accounts
{
	...
	*/
	...
	UI32 unsavedaccounts = 0;
	UI32 saveratio = 0;
	UI32 lasttimecheck, accounts::lastusedacctnum;
	ACCOUNT_LIST acctlist;
	ACCOUNT_LIST_BY_NAME accbyname;


	/*!
	\brief finalize the  namespace, write down if there is unsaved accounts
	*/
	void finalize()
	{
	...
	*/
	bool RemoveAccount(std::string name)
	{
		...
		if ( IsOnline(acc) )
		{
			NXWSOCKET r = pointers::findCharBySerial(GetInWorld(acc))->getClient()->toInt();
