...
void Skills::StealingTarget(NXWCLIENT ps)
{
	...
	if (thief->distFrom( victim ) == 1)
	{
		...
		if ( thief->checkSkill( STEALING,0,(1000-bonus)) )
		{
			...
			{
				g_bByPass = false;
				pi->amxevents[EVENT_IONSTOLEN]->Call(pi->getSerial32(), s, victim->getSerial32());
				if (g_bByPass==true)
					return;
			}
			...
			{
				g_bByPass = false;
				victim->amxevents[EVENT_CHR_ONSTOLEN]->Call(victim->getSerial32(), s);
				if (g_bByPass==true)
					return;
			}
			*/
...
void Skills::LockPick(NXWCLIENT ps)
{
	...
	{
		g_bByPass = false;
		chest->amxevents[EVENT_IONLOCKPICK]->Call(chest->getSerial32(), s);
		if (g_bByPass==true)
			return;
	}
	*/
